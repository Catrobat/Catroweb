document.addEventListener('DOMContentLoaded', function () {
  ...
  expandButtons.forEach(expandButton => {
    expandButton.addEventListener('click', () => {
      ...
      if (additionalInfoSection.style.display === 'none' || additionalInfoSection.style.display === '') {
...
function resetCloseState() {
  const url = '{{ path("reset_close_state") }}';
  fetch(url, {
    method: 'POST',
  })
    .then(response => {
      if (!response.ok) {
        console.error('Failed to reset close state');
      }
    })
    .catch(error => {
      console.error('Error:', error.message);
