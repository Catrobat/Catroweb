...
{
  public function __construct(ManagerRegistry $managerRegistry, protected RequestHelper $app_request, protected FeatureFlagManager $feature_flag_manager, private readonly TransformedFinder $project_finder)
  {
    parent::__construct($managerRegistry, Project::class);
  }

  public function getProjectByID(string $project_id, bool $include_private = false): array
  {
    $query_builder = $this->createQueryBuilder('e');
    $query_builder
      ->where($query_builder->expr()->eq('e.id', $query_builder->expr()->literal($project_id)))
    ;
  ...
  public function getProjects(string $flavor = null, string $max_version = '', int $limit = 20, int $offset = 0, string $order_by = '', string $order = 'DESC'): array
  {
    if ($this->feature_flag_manager->isEnabled('GET_projects_elastica')) {
      ...
      return $this->project_finder->find($query, $limit, ['from' => $offset]);
    }
   ...
   */
  public function filterExistingProjectIds(array $project_ids): array
  {
  ...
  public function findNext(string $previous_project_id): mixed
  {
    ...
    return $query_builder
      ->select('min(p.id)')
      ->where($query_builder->expr()->gt('p.id', ':previous_project_id'))
      ->setParameter('previous_project_id', $previous_project_id)
      ->distinct()
      ->getQuery()
  ...
  public function getProjectDataByIds(array $project_ids): array
  {
    $query_builder = $this->createQueryBuilder('e');
  ...
  public function getMostLikedProjects(?string $flavor, string $max_version, int $limit = 0, int $offset = 0): array
  {
    $query_builder = $this->createQueryBuilder('e');
    $query_builder = $this->excludeUnavailableAndPrivateProjects($query_builder, $flavor, $max_version);
    $query_builder = $this->setPagination($query_builder, $limit, $offset);
    ...
    $query_builder
      ->select(['e as project', 'COUNT(e.id) as like_count'])
      ->innerJoin(ProjectLike::class, 'l', Join::WITH,
        $query_builder->expr()->eq('e.id', 'l.project_id')->__toString())
    ...
    return array_map(fn ($result) => $result['project'], $results);
  }
  ...
  public function filterVisibleProjects(array $projects, string $max_version = ''): array
  {
    if (empty($projects)) {
      return [];
    }

    /** @var Project[] $filtered_projects */
    $filtered_projects = [];

    foreach ($projects as $project) {
      if (true === $project->getVisible() && false === $project->getPrivate()
        && ($this->app_request->isDebugBuildRequest() || false === $project->isDebugBuild())
        && ('' === $max_version || $project->getLanguageVersion() <= $max_version)) {
