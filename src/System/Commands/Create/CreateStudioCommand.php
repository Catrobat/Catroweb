  ...
  public function __construct(
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        try {
            ...
            if ($ret==1){ $output->writeln('User not found.');}
    ...
    private function createStudio(User $admin, string $name, string $description, bool $is_public, bool $is_enabled, array $usernames, array $statuses, bool $allow_comments, string $cover_path = null): int {
        ...
        $joinRequest = $this->studioManager->setJoinRequest($admin, $studio, StudioJoinRequest::STATUS_JOINED);

        if (!is_null($joinRequest))
        {
            $this->studioManager->addJoinRequest($joinRequest, $studio);
        }
        if ($is_public) {
            foreach (array_combine($usernames, $statuses) as $username => $status) {
                ...
                $joinRequest = $this->studioManager->setJoinRequest($user, $studio,StudioJoinRequest::STATUS_JOINED);
        ...
        else
        {
            foreach (array_combine($usernames, $statuses) as $username => $status) {
                ...
                $joinRequest = $this->studioManager->setJoinRequest($user, $studio, (string)$status);
                if (!is_null($joinRequest)) {
                    $this->studioManager->addJoinRequest($joinRequest, $studio);
                }
            }
