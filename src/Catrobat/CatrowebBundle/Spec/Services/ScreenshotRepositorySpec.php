...
class ScreenshotRepositorySpec extends ObjectBehavior
{
  ...
  {
    $this->shouldHaveType('Catrobat\CatrowebBundle\Services\ScreenshotRepository');
  }

  function it_throws_an_exception_if_given_an_valid_screenshot_directory()
  {
    $this->shouldThrow('Catrobat\CatrowebBundle\Exceptions\InvalidStorageDirectoryException')->during('__construct', array(__DIR__ . "/invalid_directory/", "", $this->thumbnail_dir, ""));
  }

  function it_throws_an_exception_if_given_an_valid_thumbnail_directory()
  {
    $this->shouldThrow('Catrobat\CatrowebBundle\Exceptions\InvalidStorageDirectoryException')->during('__construct', array($this->screenshot_dir, "", __DIR__ . "/invalid_directory/", ""));
  }
