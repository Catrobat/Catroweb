...
Feature: Downloaded program statistics
  ...
  Scenario: Download statistics should be persisted to database after successful download of a program
    ...
    Then the program download statistic should have a download timestamp, street, postal code, locality, latitude of approximately "48", longitude of approximately "16" and the following statistics:
      | ip              | country_code | country_name | program_id | username |
      | 88.116.169.222  | AT           | AUSTRIA      | 1          | anon.    |
