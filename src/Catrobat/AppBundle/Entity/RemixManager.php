...
class RemixManager
{
     ...
     */
    public function __construct($entity_manager, $program_repository, $scratch_program_repository, $program_remix_repository,
    ...
    public function filterExistingScratchProgramIds($scratch_ids)
    {
        ...
        return array_map(function ($data) { return $data['id']; }, $scratch_program_data);
    }
     ...
     */
    private function createNewCatrobatRemixRelations(Program $program, Program $parent_program, &$all_program_remix_relations) {
        ...
        $all_parent_ancestor_relations = $this->program_remix_repository->findBy(['descendant_id' => $parent_program->getId()]);
    ...
    private function updateProgramRemixRelations(Program $program, array $remixes_data)
    {
        ...
        // catrobat parents:
        $catrobat_remixes_data = array_filter($remixes_data, function ($remix_data) { return !$remix_data->isScratchProgram(); });
        $new_unfiltered_catrobat_parent_ids = array_map(function($remix_data) { return $remix_data->getProgramId(); }, $catrobat_remixes_data);
        ...
        $old_forward_parent_relations = array_filter($old_forward_ancestor_relations, function ($r) { return ($r->getDepth() == 1); });
        $old_forward_parent_ids = array_map(function($r) { return $r->getAncestorId(); }, $old_forward_parent_relations);
        ...
        $old_backward_parent_relations = array_filter($old_backward_ancestor_relations, function ($r) { return ($r->getDepth() == 1); });
        $old_backward_parent_ids = array_map(function($r) { return $r->getParentId(); }, $old_backward_parent_relations);
        ...
        $old_immediate_scratch_parent_ids = array_map(function($r) { return $r->getScratchParentId(); }, $old_scratch_parent_relations);

        $scratch_remixes_data = array_filter($remixes_data, function ($remix_data) { return $remix_data->isScratchProgram(); });
        $new_scratch_parent_ids = array_map(function($remix_data) { return $remix_data->getProgramId(); }, $scratch_remixes_data);
